...
## PTA 策略优化研究报告（2026-04）

> 基于 PTA（TA0）2025-01-08 ~ 2026-04-08 约 15 个月真实数据，共测试 **50+ 组参数组合**。
...
| 止盈设置 | 交易 | 胜率 | 收益率 | 最大回撤 | 盈亏比 | 夏普比 |
|:---|:---:|:---:|:---:|:---:|:---:|:---:|
| 8× ATR（基准） | 29 | 62.1% | +11.27% | 2.79% | 1.41 | 4.85 |
| 固定 2.0% | 29 | 62.1% | +11.27% | 2.79% | 1.41 | 4.85 |
| 固定 1.5% | 29 | 62.1% | +11.27% | 2.79% | 1.41 | 4.85 |
| 固定 1.0% | 29 | 62.1% | +11.27% | 2.79% | 1.41 | 4.85 |

> **原因**：日内模式收盘前自动平仓，止盈/止损根本来不及触发，所有交易结果完全一样。

#### 波段模式结论：**止损比止盈重要得多**

| 止损 | 交易 | 胜率 | 收益率 | 最大回撤 | 盈亏比 |
|:---|:---:|:---:|:---:|:---:|:---:|
| 0.5× ATR | 16 | 12.5% | +37.0% | 10.4% | 28.59 |
| 1.0× ATR | 10 | 20.0% | +54.1% | 12.4% | 20.50 |
| **1.5× ATR（基准）** | 8 | 25.0% | **+51.6%** | **14.6%** | 12.93 |
| 2.0× ATR | 6 | 33.3% | +53.4% | 13.4% | 9.60 |
| 2.5× ATR | 5 | 40.0% | **+55.7%** | **11.3%** | 8.60 |

> ⚠️ 波段模式总共只有 5~16 笔交易，样本量太小，结论仅供参考。
...
| 入场时机 | 交易 | 胜率 | 均收益 | 合计收益 |
|:---|:---:|:---:|:---:|:---:|
| 价格在 SMA30 **上方 0~+0.5%** | **16 笔** | **69%** | +0.55% | **+8.81%** |
| 价格在 SMA30 **下方 -0.5%~0%** | 5 笔 | 60% | +0.36% | +1.80% |
| 价格在 SMA30 **下方 -1.0%~-0.5%** | 8 笔 | 50% | +0.08% | +0.65% |

> **结论**：策略本质是「**趋势中的均值回归**」——只在已确认多头趋势（价格>SMA30）中入场，越接近均线的回踩点越好。

---

### 三、成交量过滤测试

| 过滤条件 | 交易 | 胜率 | 收益率 | 最大回撤 | 夏普比 |
|:---|:---:|:---:|:---:|:---:|:---:|
| **无过滤（基准）** | **29** | 62.1% | **+11.27%** | 2.79% | **4.85** |
| 成交量 > 0.5× 均量 | 18 | **66.7%** | +13.99% | 2.79% | 2.28 |
| 成交量 > 1.0× 均量 | 1 | 100% | +5.53% | 0% | 1.04 |

> **结论**：成交量 > 0.5× 均量的低量过滤可将交易从 29 笔减至 18 笔，同时将胜率从 62% 提升至 67%、收益率从 +11.27% 提升至 +13.99%，但夏普比下降。

---

### 四、更短 SMA 周期测试

| SMA 周期 | 交易 | 胜率 | 收益率 | 最大回撤 | 夏普比 |
|:---|:---:|:---:|:---:|:---:|:---:|
| **SMA30（基准）** | **29** | **62.1%** | **+11.27%** | **2.79%** | **4.85** |
| SMA25 | 27 | 55.6% | +7.68% | 2.44% | 2.74 |
| SMA20 | 31 | 45.2% | **-6.64%** | 10.29% | -2.79 |
| SMA15 | 33 | 42.4% | -0.66% | 6.66% | -0.28 |
| SMA10 | 38 | 57.9% | +3.60% | 5.39% | 1.37 |

> **结论**：SMA 越短，产生的假信号越多，胜率和收益显著下降。SMA30 是最优选择。
...
|:---|:---|:---|
| 止盈参数（日内） | **无效**，从不触发 | 不改 |
| SMA 周期 | **SMA30 最优**，更短周期显著更差 | 不改 |
| 止损参数（日内） | **无效**，收盘自动平仓 | 不改 |
| 止损参数（波段） | 2.0× ATR 可考虑，回撤更小 | 可试 |
...
```python
# 波段模式（谨慎参考，样本量小）
atr_stop = 2.0  # 比基准 1.5 更宽，最大回撤更小
```
